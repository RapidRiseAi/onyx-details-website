 ...
 * 3) Completed bookings can be archived via Done? checkbox.
 */
...
function archiveCompletedBookings() {
  ...
  archive.getRange(archive.getLastRow() + 1, 1, rowsToArchive.length, HEADERS.length).setValues(rowsToArchive);

  // Keep checkbox validation in archive for status visibility.
  const archiveStartRow = archive.getLastRow() - rowsToArchive.length + 1;
  archive.getRange(archiveStartRow, 1, rowsToArchive.length, 1).insertCheckboxes();
